...
<section class="flex md:flex-row md:justify-between flex-col ">
    ...
    <div id="checkout" class="md:pl-8 px-4 md:w-[540px] w-full  h-auto ">
        ...
        <header>
            <span class="nama-produk block md:text-xl text-lg text-primaryDark font-medium">${nama}</span>
        </header>
        ...
        <!-- CTA Buttons -->
        <div  class="md:gap-y-4 w-full flex flex-col items-center gap-y-2">
            <div 
                class=" w-full text-center bg-light rounded-2xl py-[12px] text-primaryRed border-[1px] border-primaryRed font-bold">+
                Keranjang</div>
            <a
                target="_blank"
                class="checkout bg-primaryRed text-light flex items-center justify-center w-full gap-x-[10px] py-[12px] rounded-2xl">
...
function inputJumlah() {
    ...
    // Default value untuk query string
    checkout(nama.textContent, input.value, hargaAwal.value)
    ...
    input.addEventListener('input', (e) => {
        ...
        checkout(nama.textContent, input.value, subtotal.textContent)
    })
...
function checkout (nama, jumlah, harga) {
    const checkout = document.querySelector('.checkout')
    console.log(checkout)
    checkout.href = `https://api.whatsapp.com/send/?phone=6283847841279&text=Halo kantin RTU, saya ingin pesan ${jumlah} buah ${nama}, dengan total harga Rp. ${harga}&type=phone_number&app_absent=0`;
}
